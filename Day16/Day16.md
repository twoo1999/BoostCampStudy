...
- 네트워크형 DB는 계층형 모델을 개선하여 그래프 구조를 기반으로 구성됩니다.

- 정보끼리 서로 연결되어 망 형태를 이루는 구조라고 보면 됩니다.

- 장점

  - 계층 구조에 링크를 추가해서 유연성과 접근성을 높였습니다.
  - 데이터 엑세스가 빠릅니다.

- 단점
  - 구조가 복잡해져 유지보수가 어렵습니다.
...
- 이러한 RDB를 저장, 수정, 삭제 및 검색할 수 있도록 하는 언어가 SQL입니다.

- 모든 테이블에는 각자의 고유한 키가 있으며 다른 테이블의 키를 참조하여 테이블간의 관계를 만들어낼 수 있음

- 장점
  - 시스템에 영향을 주지 않고 테이블의 관계를 추가 또는 삭제하여 데이터를 변경할 수 있습니다.
  - 여러 사용자가 동시에 데이터를 운영하고 액세스할 수 있습니다.
  - 데이터의 중복성을 줄일 수 있습니다.

#### 객체 지향형 DB

- 객체지향 프로그래밍을 기반으로 하는 모델입니다.
- 특수 전문분야 정도에서만 사용되고 있습니다.

#### 객체 관계형 DB

- 관계형 데이터베이스에 객체 지향 개념을 도입하여 만든 데이터베이스 모델입니다.

여기서 가장 많이 사용하는 데이터베이스로는 관계형 DB가 있습니다.

### 트랜잭션

트랜잭션이란 데이터베이스의 상태를 변화시키기 위한 작업의 단위를 뜻 합니다.

SQL를 이용하여 데이터베이스를 접근합니다.
...
작업단위란 논리적 기능을 수행하기 위한 작업의 단위 또는 한꺼번에 수행되어야하는 일련의 연산들을 의미합니다.

즉 게시판을 예로 들면

게시글을 수정하고 업데이트 하기 위해 UPDATE와 SELECT를 사용합니다. 이 때 UPDATE와 SELECT가 각각 따로 작업단위가 아닌 UPDATE와 SELECT가 하나의 작업단위가 되는 것 입니다.

#### 트랜잭션의 성질

- 원자성
  - 트랜잭션의 연산은 데이터베이스에 모두 반영되거나 모두 반영되지 말아야합니다.
  - 트랜잭션의 명령 중 완벽히 수행되지 않았다면 진행중이던 모든 트랜잭션은 취소되어야합니다.
- 일관성
  - 트랜잭션이 실행을 성공하면 언제나 일관성 있는 데이터베이스 상태로 변환합니다.
  - 시스템이 가지고 있는 고정요소는 트랜잭션 수행 전과 후의 상태가 같아야합니다.
  - 예를 들어 트랜잭션을 수행하는 도중 데이터베이스가 업데이트된다면 업데이트된 데이터베이스로 트랜잭션을 수행하는 것이 아닌 업데이트 전 데이터베이스로 트랜잭션을 수행합니다.
  - 이를 통해 데이터베이스의 일관성을 유지합니다.
- 독립성
  - 둘 이상의 트랜잭션이 병렬 실행되는 경우 각각의 트랜잭션은 서로에게 영향을 끼칠 수 없다.
  - 수행 중인 트랜잭션은 완전히 완료되기 전에 다른 트랜잭션의 수행결과를 참조할 수 없다.
- 지속성
  - 성공한 트랜잭션 연산의 결과는 시스템이 고장나더라도 영구적으로 반영되어야한다.

### 트랜잭션의 연산

#### commit

Commit 연산은 트랜잭션이 성공적으로 완료됐고 데이터베이스가 일관성있는 상태에 있을 때 하나의 트랜잭션이 끝났다라는 것을 알려주기 위해 사용하는 연산입니다.

#### Rollback

이 연산은 하나의 트랜잭션 처리가 비정상적으로 종료되어 데이터베이스의 일관성을 깨뜨렸을 때 트랜잭션의 일관성을 위해 트랜잭션을 위해 수행한 모든 연산을 취소하는 것입니다.

### 트랜잭션의 상태

![캡처](/image/관계형.png)

[이미지 출저]https://velog.io/@chldppwls12/%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98%EC%9D%B4%EB%9E%80

- 활동 : 트랜잭션이 실행 중인 상태

- 실패 : 트랜잭션 실행에 오류가 발생되 중단된 상태

- 철회 : 트랜잭션이 비정상적으로 종료되어 Rollback 연산을 수행한 상태

- 부분완료 : 트랜잭션의 마지막 연산까지 실행했고 commit연산이 실행되기 직전의 상태

- 완료 : 트랜잭션이 성공적으로 종료되어 commit연산을 실행한 후의 상태

[참고] https://velog.io/@chldppwls12/%ED%8A%B8%EB%9E%9C%EC%9E%AD%EC%85%98%EC%9D%B4%EB%9E%80
