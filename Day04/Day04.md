...
6. 이 과정에서 메모리를 사용하는 데이터는 다시 이동을 하게 되는데 이 때 두가지로 나뉩니다. 첫번째는 기존에 먼저 처리가 된 데이터(A, B, C) 두번째는 처음 처리되는 데이터(F, G)
  
   A, B, C 중 메모리를 사용하고 있는 데이터는 Old 영역으로 들어가게 되고 F, G 중에서도 메모리를 사용하고 있는 데이터는 다시 semi-space(to)로 들어가게 됩니다.
