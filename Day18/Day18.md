...
원래는 쿠키, 세션, 토큰 등을 통해 로그인 정보를 알고 있고 그래서 새로고침을 하거나 나갔다 들어와도 로그인 상태가 계속 유지된다고 생각하고 있었습니다. 물론 공부를 하지 않았던 부분이라 정보가 불확실했습니다.

일단은 그렇게 생각하고 넘어갔는데 캠퍼분께서 로그인정보는 확실히 서버에서 보관하는 것이 좋다고 말씀하셨습니다.

그렇게 생각하니 계속 서버에서 로그인했는지 안했는지에 대해 알고 있는 것이 과연 효율적인가를 생각했습니다. 그래서 이번 학습정리에서는 그 부분에 대해서 정리하고자합니다.

또한

HTTP를 통해 통신한다는 것은 알았는데 중요한 것은 보안이죠. HTTPS라던지 실제 시스템에서는 보안에 대해 어떻게 신경을 쓰는지 확인하고 싶어지네요.

또한 캠퍼분의 개선 중 암호화에 대한 이야기가 나왔는데 이 점 또한 생각해봐야겠습니다. 실제로는 body를 암호화하고 데이터를 보낸다는데 이전에 배운 SHA 라든지 아니면 머리 속에 애매하게 떠다니고 있는 대칭/비대칭 키라던지 이런 부분을 더 자세하게 배워야겠습니다.

이번 학습정리는 위의 내용을 포함하는 전반적인 인터넷 통신에 대한 이야기가 될거같네요 .
